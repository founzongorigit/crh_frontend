...
const emit = defineEmits<{
  save: [payload: ValidationCreatePayload | ValidationUpdatePayload]
}>()
...
const isEditMode = computed(() => !!props.validation)
const title = computed(() => (isEditMode.value ? 'Modifier la validation' : 'Nouvelle validation'))
...
watch(
  ...
  ([validation]) => {
    ...
    form.projet_id = validation?.projet_id ?? ''
    form.niveau = validation?.niveau ?? 1
...
const submit = () => {
  const payload = {
    ...
    ...(isEditMode.value ? {} : { projet_id: form.projet_id, niveau: form.niveau }),
  }
  ...
  <Dialog v-model:visible="visible" :header="title" modal class="validation-dialog">
    <form class="validation-form" @submit.prevent="submit">
      <div v-if="!isEditMode" class="form-field">
        ...
        <InputText id="projet_id" v-model="form.projet_id" required />
      </div>
      ...
      <div class="form-field form-field-wide">
        ...
        <a
          v-else-if="form.existing_pj_url"
          class="file-hint"
          :href="form.existing_pj_url"
          target="_blank"
          rel="noreferrer"
        >
          Document actuel
