...
const pieceJointeUrl = computed(() => getBackendFileUrl(props.validation?.pj_url))
...
<template>
  <Dialog v-model:visible="visible" header="Détail de la validation" modal class="validation-dialog" dismissable-mask>
    <div v-if="validation" class="validation-details">
      ...
      <div>
        <span class="detail-label">Crée le</span>
        <strong>{{ formatDate(validation.created_at) }}</strong>
      </div>
      <div>
        <span class="detail-label">Traité le</span>
        <strong>{{ formatDate(validation.updated_at) }}</strong>
      </div>
      <div>
      ...
      <div class="details-wide">
        ...
        <a v-if="pieceJointeUrl" :href="pieceJointeUrl" target="_blank" rel="noreferrer">
          Voir le document
        </a>
